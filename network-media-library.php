...
/**
 * Returns whether we're currently on the network media library site, regardless of any switching that's occurred.
 *
 ...
 */
function prevent_attaching (): void
{
    unset( $_REQUEST['post_id'] );
}

/**
 * Enables the featured image feature for the network media library site.
 */
add_action( 'enable_post_type_featured_images_for_network', __NAMESPACE__ . '\enable_post_type_featured_images_for_network', 10, 1 );

function enable_post_type_featured_images_for_network ( $post_types ): void
{
    foreach ( $post_types as $post_type ) {
        add_action( 'rest_insert_' . $post_type, function ( $post, $request, $update ) {
